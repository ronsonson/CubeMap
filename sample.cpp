...
void Get_Mouse(int button, int state, int x, int y)
{
	int mouseX = x;
	int mouseY = y;
	fprintf(stderr, "Mouse position: %d %d", x, y);
}
...
float uEta = 1.33;
float uWhiteMix = 0.;
float NowuMix, NowuNoiseAmp, NowuNoiseFreq;
...
int posXTex, negXTex, posYTex, negYTex, posZTex, negZTex;
char* FaceFiles[6] =
{
	"posx.bmp",
	"negx.bmp",
...
Display( )
{
	...
	gluLookAt( 0.f, 1.f, 0.5f,     0.f, -2.5f, 0.f,     0.f, 1.f, 0.f );
	...
	Pattern.SetUniformVariable((char*)"uMix", NowuMix);
	Pattern.SetUniformVariable((char*)"uNoiseAmp", NowuNoiseAmp);
	Pattern.SetUniformVariable((char*)"uNoiseFreq", NowuNoiseFreq);
	Pattern.SetUniformVariable((char*)"uWaves", NowuWaves);
...
InitGraphics( )
{
	...
	if (texture == NULL)
	{
		fprintf(stderr, "Noise texture IS NOT working!");
	}
	else
	{
		fprintf(stderr, "Noise texture IS working!");
	}
	...
	{
		int width, height;
		unsigned char* texture = BmpToTexture(FaceFiles[i], &width, &height);
		if (texture == NULL)
			fprintf(stderr, "Cannot open texture '%s'\n", FaceFiles[i]);
		else
			fprintf(stderr, "Opened '%s': width = %d ; height = %d\n", FaceFiles[i], width, height);
...
InitLists( )
{
	...
	float dy = ymax - ymin;
	float z = 0.f;
	int numy = 256;		// set this to what you want it to be
	int numx = 256;		// set this to what you want it to be
	...
	CubeLists[0] = glGenLists(1);
	glNewList(CubeLists[0], GL_COMPILE);
	// Right Face
	glEnable(GL_TEXTURE_2D);
	glBegin(GL_QUADS);
	glBindTexture(GL_TEXTURE_2D, CubeTextures[0]);
	...
	CubeLists[1] = glGenLists(1);
	glNewList(CubeLists[1], GL_COMPILE);
	glEnable(GL_TEXTURE_2D);
	glBegin(GL_QUADS);
	// Left face
	glBindTexture(GL_TEXTURE_2D, CubeTextures[1]);
	...
	CubeLists[2] = glGenLists(1);
	glNewList(CubeLists[2], GL_COMPILE);
	glEnable(GL_TEXTURE_2D);
	glBegin(GL_QUADS);
	...
	CubeLists[3] = glGenLists(1);
	glNewList(CubeLists[3], GL_COMPILE);
	glEnable(GL_TEXTURE_2D);
	glBegin(GL_QUADS);
	// Bottom face
	glBindTexture(GL_TEXTURE_2D, CubeTextures[3]);
	...
	CubeLists[4] = glGenLists(1);
	glNewList(CubeLists[4], GL_COMPILE);
	glEnable(GL_TEXTURE_2D);
	glBegin(GL_QUADS);
	// Front face
	glBindTexture(GL_TEXTURE_2D, CubeTextures[4]);
	...
	CubeLists[5] = glGenLists(1);
	glNewList(CubeLists[5], GL_COMPILE);
	glEnable(GL_TEXTURE_2D);
	glBegin(GL_QUADS);
	// Back face
	glBindTexture(GL_TEXTURE_2D, CubeTextures[5]);
